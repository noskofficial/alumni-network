      ...
      <div class="flex justify-center flex-wrap md:flex-nowrap" :class="direction">
        <div class="flex flex-col">
          <h3 class="text-3xl text-white mt-5">{{ title }}
            <h2 class="text-2xl text-white mt-2">{{ titleEvent }}</h2>
          </h3>
          <p class="text-white mt-4 w-[90%]">{{ description }}</p>
        ...
        <div class="w-[100%] bg-cover mb-1">
          <img :src="image" alt="" class="w-[600px] h-[90%] my-5">
        </div>
      </div>
    </div>
  </div>
...
const { title, titleEvent, description, image, link } = defineProps({
  title: String,
  titleEvent: String,
  description: String,
  image: String,
